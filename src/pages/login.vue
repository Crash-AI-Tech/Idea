<template>
  <div class="min-h-screen flex items-center justify-center bg-dark-bg">
    <div class="bg-dark-card p-8 rounded-lg shadow-md w-full max-w-md">
      <h2 class="text-2xl font-bold mb-6 text-center text-light-text">{{ $t('auth.loginTitle') }}</h2>
      ...
      <div class="relative flex py-5 items-center">
        ...
        <span class="flex-shrink mx-4 text-medium-text">{{ $t('auth.or') }}</span>
        <div class="flex-grow border-t border-gray-600"></div>
      </div>
      ...
      <p class="mt-6 text-center text-medium-text text-sm">
        ...
        <NuxtLink to="/register" class="font-bold text-primary-green hover:text-green-400">{{ $t('auth.registerLink') }}</NuxtLink>
      </p>
    </div>
  </div>
</template>
...
const loginWithEmail = async () => {
  ...
  try {
    ...
    errorMsg.value = error.message || t('auth.loginError')
  }
}
</script>